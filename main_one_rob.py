...
def plot_pretrain_curves(rob_curve_path,rob_curve_dir,ma_window=20):
    ...
    2. Train / Validation Robust minimum-user rate
    3. 每個UE分開畫SINR power components，共8條線：
       Train / Val signal
    ...
    plt.plot(epochs,train_loss,label="Train Loss",alpha=0.35,linewidth=1.0)
    plt.plot(epochs,val_loss,label="Validation Loss",alpha=0.35,linewidth=1.0)
    ...
    if len(train_loss_ma) > 0:
        plt.plot(train_ma_epochs,train_loss_ma,label=f"Train Loss MA({ma_window})",linewidth=2.5)

    if len(val_loss_ma) > 0:
        plt.plot(val_ma_epochs,val_loss_ma,label=f"Validation Loss MA({ma_window})",linewidth=2.5)
    ...
    fig1_path = os.path.join(rob_curve_dir,"Fig1_loss_curve.png")
    ...
    # ================================
    # Fig. 2: Robust Minimum-User Rate
    # ================================
    plt.figure(figsize=(10,6))

    plt.plot(epochs,train_robust,label="Train Robust Minimum-User Rate",alpha=0.35,linewidth=1.0)
    plt.plot(epochs,val_robust,label="Validation Robust Minimum-User Rate",alpha=0.35,linewidth=1.0)
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Robust Minimum-User Rate (bps/Hz)")
    plt.title(f"Fig. 2: One-timescale ROB Robust Minimum-User Rate (Q{OUTAGE_QUANTILE:.2f})")
    plt.grid(True,alpha=0.35)
    plt.legend()
    plt.tight_layout()

    fig2_path = os.path.join(rob_curve_dir,"Fig2_robust_curve.png")
    ...
    for ue_idx in range(num_users):
        ...
        plt.plot(epochs,train_signal_power[:,ue_idx],label="Train Signal",linewidth=1.8)
        plt.plot(epochs,val_signal_power[:,ue_idx],label="Validation Signal",linewidth=1.8,linestyle="--")

        plt.plot(epochs,train_comm_interf_power[:,ue_idx],label="Train Communication Interference",linewidth=1.8)
        plt.plot(epochs,val_comm_interf_power[:,ue_idx],label="Validation Communication Interference",linewidth=1.8,linestyle="--")

        plt.plot(epochs,train_radar_interf_power[:,ue_idx],label="Train Radar Interference",linewidth=1.8)
        plt.plot(epochs,val_radar_interf_power[:,ue_idx],label="Validation Radar Interference",linewidth=1.8,linestyle="--")

        plt.plot(epochs,train_noise_power,label="Train Noise",linewidth=1.8)
        plt.plot(epochs,val_noise_power,label="Validation Noise",linewidth=1.8,linestyle="--")

        plt.yscale("log")

        plt.xlabel("Epoch")
        plt.ylabel("Power (linear scale)")
        plt.title(f"Fig. 3: One-timescale ROB SINR Power Components — UE {ue_idx}")
        plt.grid(True,which="both",alpha=0.35)
        plt.legend()
        plt.tight_layout()

        fig3_path = os.path.join(rob_curve_dir,f"Fig3_UE{ue_idx}_sinr_power_components.png")
...
if __name__ == "__main__":
    ...
    for epoch in trange(ROB_EPOCHS, desc="ROB Training"):
        ...
        # ================================
        # Validation: 全部 val channels，不使用chunk
        # ================================
        comm_net.eval()
        radar_net.eval()
        theta_net.eval()
        ...
        with torch.no_grad():
            ...
            # 使用 .unsqueeze(0) 新增第0維度供INJ使用
            # 使用 .expand 功能上複製estimated channels成S份
            ...
            # 注入不確定：INJECTION_VARIANCE是相對通道功率
            # noise power = INJECTION_VARIANCE * mean(|channel|^2)
            ...
            # 還原shape並取輸出結果
            val_robust_allUE_rate   = val_metrics_flat["rate"].reshape(S,B,K)                   # (S,B,K) 各UE在每筆injection channel的rate
            val_robust_target_snr   = val_metrics_flat["target_snr"].reshape(S,B)               # (S,B) 每一筆injection channel的target SNR，linear

            val_robust_signal       = val_metrics_flat["signal"].reshape(S,B,K)                 # (S,B,K) 各UE的signal power
            val_robust_comm_interf  = val_metrics_flat["comm_interf"].reshape(S,B,K)            # (S,B,K) 各UE的communication interference
            val_robust_radar_interf = val_metrics_flat["radar_interf"].reshape(S,B,K)           # (S,B,K) 各UE的radar interference
    ...
    print(f"[INFO] Best validation robust = {best_val_worstUE_q05:.6f}")       # 這裡要print Best validation robust
