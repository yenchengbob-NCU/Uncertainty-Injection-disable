...
NOISE_POWER = (10 ** ((N0_dBm_per_Hz - 30) / 10.0)) * BANDWIDTH # 雜訊總功率(瓦) #3.16 × 10⁻⁴ W
...
SCENARIO_TAG = f"M{M}_K{K}_N{N_TARGET}"

# 門檻標籤：感測門檻(db)_罰則係數 λ
THR_TAG = (
    f"THR_{SENSING_SNR_THRESHOLD_dB}db_"
    f"punish_{SENSING_LOSS_WEIGHT}"    
)

SETTING_STRING = (
    f"N0_{NOISE_POWER:.5f}_"
    f"INJERR_{INJECTION_VARIANCE:.5f}"
)
