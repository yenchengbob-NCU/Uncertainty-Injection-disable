...
if __name__ == "__main__":
    ...
    comm_net.load_model(reg_comm_ckpt, verbose=False)
    radar_net.load_model(reg_radar_ckpt, verbose=False)
    theta_net.load_model(reg_theta_ckpt, verbose=False)
    ...
    with torch.no_grad():
        ...
        # 輸入網路
        theta_reg   = theta_net(h_dk_teat,h_rk_teat,G_test,g_dt_test)
        W_C_reg_raw = comm_net(h_dk_teat,h_rk_teat,G_test,g_dt_test)
        W_R_reg_raw = radar_net(h_dk_teat,h_rk_teat,G_test,g_dt_test)
        ...
        # 複製theta
        theta_rep = theta_reg.unsqueeze(0).expand(S, B, N)

        # 複製W_C & W_R
        W_C_rep = W_C_reg.unsqueeze(0).expand(S, B, M, K)
        W_R_rep = W_R_reg.unsqueeze(0).expand(S, B, M, 1)
